...
export default function USRemoteJobsPage() {
  ...
  const fetchJobs = useCallback(
    async (page: number, search: string, tech: string) => {
      ...
      try {
        ...
        console.warn("Could not load US remote jobs:", err);
        setError("US remote job listings are currently unavailable.");
        setJobs([]);
        setTotalCount(0);
        setTotalPages(1);
      } finally {
      ...
      <main className="flex-1 pt-24 pb-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-6">
            ...
            <span>/</span>
            <span className="text-foreground font-medium">Remote Jobs within US</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 border-b border-border pb-8">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                All verified remote and hybrid opportunities aggregated across the United States from official job sources.
              </p>
            </div>
            ...
            <form onSubmit={handleSearchSubmit} className="flex items-center gap-2 max-w-md w-full">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search US role or company..."
                  value={searchQuery}
          ...
          {!loading && !error && jobs.length === 0 && (
            <div className="mt-10 rounded-3xl border border-dashed border-border bg-card p-12 text-center">
              ...
              <h2 className="mt-4 text-base font-semibold">No US remote jobs found</h2>
              <p className="mt-1 text-sm text-muted-foreground max-w-md mx-auto">
                No aggregated US jobs match your current search filters. Check back soon as new provider feeds sync!
              </p>
              {(appliedSearch || selectedTech !== "all") && (
                <button
                  onClick={handleResetFilters}
